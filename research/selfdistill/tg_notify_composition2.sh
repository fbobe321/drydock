...
OUT="$SD/composition2_results.csv"; LOG="$SD/composition2.log"
TOKEN="${TG_TOKEN:-}"; CHAT="${TG_CHAT:-}"
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/home/bobef/miniconda3/bin"
for i in $(seq 1 720); do pgrep -f 'sd_train_oneshot_composition2' >/dev/null 2>&1 || break; sleep 30; done
...
if not rows: verdict="no rows — check composition2.log"
...
msg=(f"drydock self-distill — COMPOSITION2 (pruned targets) diagnostic DONE (7 P1-condensed traces, r=128, loss~{mll})\n\n"
     f"Reproduction (trained tasks): {s}/{n}\n{detail}\n\n{verdict}\n\n"
     f"(Baseline was baseline r=128 unpruned -> 2/7.) Loop relaunched: {'yes' if loop_up else 'NOT yet'}. Results: {out}")

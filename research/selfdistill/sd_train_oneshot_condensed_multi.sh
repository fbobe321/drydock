#!/usr/bin/env bash
# sd_train_oneshot_condensed_multi.sh — the HELD-OUT GENERALIZATION test (the real RSI question).
# Train ONE condensed LoRA on all 7 condensed (canary-scrubbed) traces, serve the MATCHED HF-Q5 base,
# and eval BOTH: the 6 frozen HELD-OUT tasks (never trained -> generalization) and the 7 TRAINED tasks
# (reproduction context). Held-out base=0 by construction, so any held-out solve = genuine transfer.
...
SFT="$SD/gens/condensed_multi/condensed_sft.jsonl"      # 7 rows, scrubbed
ADAPTER=/data3/compass/adapters/sd-condensed-multi
LORA_GGUF=/data3/Models/selfdistill-condensed-multi-lora.gguf
HELDOUT=(adaptive-rejection-sampler circuit-fibsqrt dna-assembly fix-ocaml-gc mteb-leaderboard regex-chess)
...
STEPS="${STEPS:-350}"; SEQ="${SEQ:-512}"; ACCUM="${ACCUM:-1}"; LORA_R="${LORA_R:-32}"; LR="${LR:-0.0002}"
LOG="$SD/condensed_multi.log"; OUT="$SD/condensed_multi_results.csv"
...
say "===== CONDENSED-MULTI START — train 7 condensed traces (steps=$STEPS r=$LORA_R); stopping loop ====="
...
say "training (condensed multi-task overfit, 7 traces)…"
...
echo "config,set,task,reward" > "$OUT"
serve "$HFQ" "$LORA_GGUF" || { say "serve failed"; setsid nohup bash "$SD/selfdistill_launch.sh" >> "$SD/selfdistill.log" 2>&1 & exit 1; }
# HELD-OUT first (the key generalization metric), then TRAINED (reproduction context)
for t in "${HELDOUT[@]}"; do
  r=$(eval_task "$t"); say "  [HELDOUT] $t -> reward=$r"; echo "condensed-multi,heldout,$t,$r" >> "$OUT"
done
for t in "${TRAINED[@]}"; do
  r=$(eval_task "$t"); say "  [TRAINED] $t -> reward=$r"; echo "condensed-multi,trained,$t,$r" >> "$OUT"
done

say "===== CONDENSED-MULTI DONE ====="; column -s, -t "$OUT" 2>/dev/null | tee -a "$LOG"

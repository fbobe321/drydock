...
SFT="$SD/gens/composition2/comp2_sft.jsonl"     # 7 rows, P1-condensed (run steps), scrubbed
ADAPTER=/data3/compass/adapters/sd-composition2
LORA_GGUF=/data3/Models/selfdistill-composition2-lora.gguf
...
LOG="$SD/composition2.log"; OUT="$SD/composition2_results.csv"
...
say "===== COMPOSITION2 (pruned targets) START — 7 P1-condensed traces, r=$LORA_R steps=$STEPS; stopping loop ====="
...
say "===== COMPOSITION2 DONE ====="; column -s, -t "$OUT" 2>/dev/null | tee -a "$LOG"

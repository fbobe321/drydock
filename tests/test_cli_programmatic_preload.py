...
from drydock.core import run_programmatic
from drydock.core.config import Backend
from drydock.core.types import LLMMessage, OutputFormat, Role
...
def test_run_programmatic_preload_streaming_is_batched(
    ...
    monkeypatch.setattr(
        "drydock.core.programmatic.create_formatter", lambda *_args, **_kwargs: spy
    )
    ...
    with mock_backend_factory(
        ...
        new_session = [
            e for e in telemetry_events if e.get("event_name") == "drydock.new_session"
        ]
        ...
        assert (
            spy.emitted[0][1] == "You are Drydock, a super useful programming assistant."
        )

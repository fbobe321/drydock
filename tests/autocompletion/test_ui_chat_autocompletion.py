...
@pytest.mark.asyncio
async def test_popup_appears_with_matching_suggestions(drydock_app: DrydockApp) -> None:
    async with drydock_app.run_test() as pilot:
        chat_input = drydock_app.query_one(ChatInputContainer)
        popup = drydock_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_popup_hides_when_input_cleared(drydock_app: DrydockApp) -> None:
    async with drydock_app.run_test() as pilot:
        popup = drydock_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_updates_selected_suggestion(drydock_app: DrydockApp) -> None:
    async with drydock_app.run_test() as pilot:
        popup = drydock_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_cycles_through_suggestions(drydock_app: DrydockApp) -> None:
    async with drydock_app.run_test() as pilot:
        popup = drydock_app.query_one(CompletionPopup)
...
async def test_pressing_enter_submits_selected_command_and_hides_popup(
    drydock_app: DrydockApp, telemetry_events: list[dict]
) -> None:
    async with drydock_app.run_test() as pilot:
        chat_input = drydock_app.query_one(ChatInputContainer)
        popup = drydock_app.query_one(CompletionPopup)
        ...
        assert popup.styles.display == "none"
        message = drydock_app.query_one(".user-command-message")
...
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / "drydock" / "acp").mkdir(parents=True)
    (tmp_path / "drydock" / "acp" / "entrypoint.py").write_text("", encoding="utf-8")
    (tmp_path / "drydock" / "acp" / "agent.py").write_text("", encoding="utf-8")
...
async def test_path_completion_popup_lists_files_and_directories(
    drydock_app: DrydockApp, file_tree: Path
) -> None:
    async with drydock_app.run_test() as pilot:
        popup = drydock_app.query_one(CompletionPopup)
...
async def test_fuzzy_matches_subsequence_characters(
    file_tree: Path, drydock_app: DrydockApp
) -> None:
    async with drydock_app.run_test() as pilot:
        popup = drydock_app.query_one(CompletionPopup)
...
async def test_finds_files_recursively_by_filename(
    ...
    async with drydock_app.run_test() as pilot:
        ...
        popup_content = str(popup.render())
        assert "@drydock/acp/entrypoint.py" in popup_content
        assert popup.styles.display == "block"

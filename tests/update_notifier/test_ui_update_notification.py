...
from tests.conftest import build_test_drydock_app, build_test_drydock_config
from tests.update_notifier.adapters.fake_update_cache_repository import (
    FakeUpdateCacheRepository,
)
from tests.update_notifier.adapters.fake_update_gateway import FakeUpdateGateway
from drydock.cli.textual_ui.app import DrydockApp
from drydock.cli.textual_ui.widgets.messages import WhatsNewMessage
...
def build_update_test_app(
    drydock_config_with_update_checks_enabled: DrydockConfig,
) -> Callable[..., DrydockApp]:
    def _build(
        *,
        ...
        return build_test_drydock_app(
            ...
            config=config or drydock_config_with_update_checks_enabled,
...
async def _wait_for_notification(
    app: DrydockApp, pilot, *, timeout: float = 1.0, interval: float = 0.05
) -> Notification:
...
@pytest.fixture
def drydock_config_with_update_checks_enabled() -> DrydockConfig:
    return build_test_drydock_config(enable_update_checks=True)


@pytest.mark.asyncio
async def test_ui_displays_update_notification(
    build_update_test_app: Callable[..., DrydockApp],
) -> None:
...
async def test_ui_does_not_invoke_gateway_nor_show_error_notification_when_update_checks_are_disabled(
    build_update_test_app: Callable[..., DrydockApp], drydock_config: DrydockConfig
) -> None:
    notifier = FakeUpdateGateway(update=Update(latest_version="0.2.0"))
    app = build_update_test_app(update_notifier=notifier, config=drydock_config)
...
async def test_ui_displays_whats_new_message_when_content_exists(
    build_update_test_app: Callable[..., DrydockApp], tmp_path: Path
) -> None:
    notifier = FakeUpdateGateway(update=None)
    cache = UpdateCache(
    ...
    with patch("drydock.cli.update_notifier.whats_new.DRYDOCK_ROOT", tmp_path):
...
async def test_ui_displays_success_notification_when_auto_update_succeeds(
    ...
    config = build_test_drydock_config(enable_update_checks=True, enable_auto_update=True)
    notifier = FakeUpdateGateway(update=Update(latest_version="0.2.0"))

    with patch("drydock.cli.update_notifier.update.UPDATE_COMMANDS", ["true"]):
...
async def test_ui_displays_update_notification_when_auto_update_fails(
    ...
    with patch("drydock.cli.update_notifier.update.UPDATE_COMMANDS", ["false"]):
        app = build_update_test_app(update_notifier=notifier, config=config)

...
async def test_ui_clipboard_notification_does_not_crash_on_markup_text(
    monkeypatch: pytest.MonkeyPatch, drydock_app: DrydockApp
) -> None:
    async with drydock_app.run_test(notifications=True) as pilot:
        await drydock_app.mount(ClipboardSelectionWidget("[/]"))
        with patch("drydock.cli.clipboard._copy_to_clipboard"):
            copy_selection_to_clipboard(drydock_app)

        await pilot.pause(0.1)
        notifications = list(drydock_app._notifications)

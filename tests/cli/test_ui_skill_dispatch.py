...
@pytest.fixture
def drydock_app_with_skills(tmp_path: Path) -> DrydockApp:
    skills_dir = tmp_path / "skills"
    skills_dir.mkdir()
    create_skill(skills_dir, "my-skill", body=SKILL_BODY)
    return build_test_drydock_app(config=build_test_drydock_config(skill_paths=[skills_dir]))


async def _wait_for_user_message_containing(
    drydock_app: DrydockApp, pilot, text: str, timeout: float = 1.0
) -> UserMessage:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        for message in drydock_app.query(UserMessage):
...
async def _wait_for_error_message_containing(
    ...
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        for error in drydock_app.query(ErrorMessage):
...
async def test_skill_without_args_sends_skill_content(
    drydock_app_with_skills: DrydockApp,
) -> None:
    async with drydock_app_with_skills.run_test() as pilot:
        await pilot.pause(0.1)
        chat_input = drydock_app_with_skills.query_one(ChatInputContainer)
        chat_input.post_message(ChatInputContainer.Submitted("/my-skill"))
        await pilot.pause(0.1)

        message = await _wait_for_user_message_containing(
            drydock_app_with_skills, pilot, "Do the thing."
        )
...
async def test_unknown_skill_falls_through_to_agent(
    ...
    async with drydock_app_with_skills.run_test() as pilot:
        ...
        skill_errors = [
            e
            for e in drydock_app_with_skills.query(ErrorMessage)
...
@pytest.mark.asyncio
async def test_bare_slash_falls_through(drydock_app_with_skills: DrydockApp) -> None:
    async with drydock_app_with_skills.run_test() as pilot:
        await pilot.pause(0.1)
        chat_input = drydock_app_with_skills.query_one(ChatInputContainer)
        ...
        assert not any(
            ...
            for m in drydock_app_with_skills.query(UserMessage)
        )
...
async def test_skill_with_missing_file_shows_error(tmp_path: Path) -> None:
    ...
    drydock_app = build_test_drydock_app(
        config=build_test_drydock_config(skill_paths=[skills_dir])
    )
    ...
    async with drydock_app.run_test() as pilot:
        ...
        error = await _wait_for_error_message_containing(
            drydock_app, pilot, "Failed to read skill file"
        )

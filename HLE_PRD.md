...
| Phase 1    | **1/18 ≈ 5%** (in flight) | same as P0v4 | worked example replaces answer; 11 with example, 9 empty |
...
**Phase 0 cycle complete.** With the literal answer in GraphRAG and drydock's authoritative-recognition + retrieval improvements, drydock + Gemma 4 26B-A4B solves 20/20 of the v1 baseline failures across all 8 categories.

**Phase 1 (worked example, in flight)**: removed the literal `ANSWER:` line and replaced with a similar-but-distinct worked example (11/20 generated by gemma4 itself; 9/20 empty due to thinking-budget exhaustion — left as natural fallback). At 18/20 done: **1/18 = ~5% correct** — basically baseline. This isolates the model-trust mechanism: when the literal answer is in retrieval, drydock can steer the model to copy it (90%+); when only a similar-but-different worked example is provided, the model can't extract the pattern and drops to baseline. **Deep Noir steering is the leverage to bridge this gap.**

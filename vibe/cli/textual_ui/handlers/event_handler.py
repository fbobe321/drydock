...
from vibe.core.drydock_states import get_state_term
...
def _nautical_status(tool_status: str) -> str:
    """Convert a tool status string to a nautical-themed status.

    Returns ``"⚓ <Term>"`` without trailing dots because the loading
    widget already appends its own animated ellipsis.
    """
    lower = tool_status.lower()
    for keyword, category in _STATUS_TO_NAUTICAL_CATEGORY.items():
        if keyword in lower:
            return f"\u2693 {get_state_term(category)}"
    # Default: use the "execute" category for unrecognised tool activity
    return f"\u2693 {get_state_term('execute')}"
...
class EventHandler:
    ...
    async def _handle_tool_call(
        ...
        if loading_widget and event.tool_class:
            ...
            loading_widget.set_status(_nautical_status(adapter.get_status_text()))
